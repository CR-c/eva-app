  ...
  const loadingContent = (
    <View className={`${containerClasses} ${className}`}>
      ...
      <View className={`${sizeClasses[size]}`}>
        <Loading type={type} className='text-blue-500' />
      </View>

      {/* 加载文本 */}
      {text && <Text className='text-gray-500 text-sm mt-3 text-center'>{text}</Text>}
    </View>
  )
  ...
  if (overlay) {
    return (
      <View className='fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50'>
        <View className='bg-white rounded-lg p-6 shadow-lg min-w-32'>{loadingContent}</View>
      </View>
    )
  }
